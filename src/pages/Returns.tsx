...
const Returns = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      const { data } = await supabase.from("cms_pages").select("content").eq("page_key", "returns").eq("is_active", true).single();
  ...
  const returnSteps = [
    { step: 1, titleKey: "returns.step1Title", descKey: "returns.step1Desc" },
    { step: 2, titleKey: "returns.step2Title", descKey: "returns.step2Desc" },
    { step: 3, titleKey: "returns.step3Title", descKey: "returns.step3Desc" },
    { step: 4, titleKey: "returns.step4Title", descKey: "returns.step4Desc" },
  ];

  const returnableCategories = [
    { catKey: "categories.electronics", periodKey: "returns.electronics14", condKey: "returns.electronicsCondition" },
    { catKey: "categories.fashion", periodKey: "returns.fashion30", condKey: "returns.fashionCondition" },
    { catKey: "categories.homeLiving", periodKey: "returns.home30", condKey: "returns.homeCondition" },
    { catKey: "categories.beauty", periodKey: "returns.beauty7", condKey: "returns.beautyCondition" },
    { catKey: "categories.toysGames", periodKey: "returns.toys30", condKey: "returns.toysCondition" },
  ];

  const eligibleItems = ["returns.eligible1", "returns.eligible2", "returns.eligible3", "returns.eligible4", "returns.eligible5"];
  const nonReturnableItems = ["returns.nonReturnable1", "returns.nonReturnable2", "returns.nonReturnable3", "returns.nonReturnable4", "returns.nonReturnable5", "returns.nonReturnable6", "returns.nonReturnable7"];

  const refundFaqs = [
    { qKey: "returns.refundQ1", aKey: "returns.refundA1" },
    { qKey: "returns.refundQ2", aKey: "returns.refundA2" },
    { qKey: "returns.refundQ3", aKey: "returns.refundA3" },
    { qKey: "returns.refundQ4", aKey: "returns.refundA4" },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1">
        <section className="bg-gradient-to-br from-primary/10 to-primary/5 py-16">
          <div className="container mx-auto px-4 text-center">
            <h1 className="text-4xl md:text-5xl font-heading font-bold mb-4">{t('returns.title')}</h1>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">{t('returns.subtitle')}</p>
          </div>
        </section>

        <section className="py-16 container mx-auto px-4">
          <h2 className="text-2xl font-heading font-bold text-center mb-12">{t('returns.howToReturn')}</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            {returnSteps.map((step) => (
              <Card key={step.step} className="text-center relative">
                <CardHeader>
                  <div className="w-12 h-12 bg-primary text-primary-foreground rounded-full flex items-center justify-center mx-auto mb-3 text-xl font-bold">{step.step}</div>
                  <CardTitle className="text-lg">{t(step.titleKey)}</CardTitle>
                </CardHeader>
                <CardContent><p className="text-sm text-muted-foreground">{t(step.descKey)}</p></CardContent>
              </Card>
            ))}
          </div>
          <div className="text-center mt-8">
            <NavLink to="/orders">
              <Button size="lg" className="bg-primary hover:bg-primary-hover">
                <RotateCcw className="h-4 w-4 ltr:mr-2 rtl:ml-2" />{t('returns.startReturn')}
              </Button>
            </NavLink>
          </div>
        </section>
        ...
        <section className="py-16 bg-muted/30">
          <div className="container mx-auto px-4">
            <h2 className="text-2xl font-heading font-bold text-center mb-8">{t('returns.returnPeriodsByCategory')}</h2>
            <div className="max-w-3xl mx-auto">
              <Card>
                <CardContent className="p-0">
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr className="border-b bg-muted/50">
                          <th className="text-left p-4 font-semibold">{t('returns.categoryCol')}</th>
                          <th className="text-left p-4 font-semibold">{t('returns.returnPeriod')}</th>
                          <th className="text-left p-4 font-semibold">{t('returns.condition')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {returnableCategories.map((item) => (
                          <tr key={item.catKey} className="border-b last:border-0">
                            <td className="p-4 font-medium">{t(item.catKey)}</td>
                            <td className="p-4"><span className="inline-flex items-center gap-1 text-primary"><Clock className="h-4 w-4" />{t(item.periodKey)}</span></td>
                            <td className="p-4 text-muted-foreground">{t(item.condKey)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        <section className="py-16 container mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <Card>
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <CardTitle>{t('returns.eligibleForReturn')}</CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                <ul className="space-y-3">
                  {eligibleItems.map((key) => (
                    <li key={key} className="flex items-start gap-2"><CheckCircle className="h-4 w-4 text-green-600 mt-1 shrink-0" /><span className="text-muted-foreground">{t(key)}</span></li>
                  ))}
                </ul>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center"><XCircle className="h-5 w-5 text-red-600" /></div>
                  <CardTitle>{t('returns.nonReturnableItems')}</CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                <ul className="space-y-3">
                  {nonReturnableItems.map((key) => (
                    <li key={key} className="flex items-start gap-2"><XCircle className="h-4 w-4 text-red-600 mt-1 shrink-0" /><span className="text-muted-foreground">{t(key)}</span></li>
                  ))}
                </ul>
              </CardContent>
            </Card>
        ...
        <section className="py-16 bg-muted/30">
          <div className="container mx-auto px-4">
            <h2 className="text-2xl font-heading font-bold text-center mb-8">{t('returns.refundInfo')}</h2>
            <div className="max-w-3xl mx-auto">
              <Accordion type="single" collapsible className="space-y-4">
        ...
        <section className="py-16 container mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-2 text-amber-600 mb-4">
            <AlertTriangle className="h-5 w-5" /><span className="font-medium">{t('returns.needHelpReturn')}</span>
          </div>
          <p className="text-muted-foreground mb-6 max-w-xl mx-auto">{t('returns.needHelpReturnDesc')}</p>
          <NavLink to="/contact"><Button size="lg" variant="outline">{t('common.contactSupport')}</Button></NavLink>
        </section>
      </main>
      <Footer />
    </div>
